...
export default function ContentPage() {
    ...
    const togglePublish = async (id: string, currentStatus: boolean) => {
        try {
            const res = await fetch(`/api/internal/content/${id}`, {
                method: "PATCH",
    ...
    if (loading) {
        return <div className="p-6">Loading content blocks...</div>;
    }
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="space-y-1">
                    ...
                    <h1 className="text-2xl font-bold text-slate-900">Content Blocks</h1>
                </div>
                <Link
                    href="/internal/content/create"
                            ...
                            {contentBlocks.map((block) => (
                                <tr key={block.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-slate-900">
                                        ...
                                        {block.subtitle && (
                                            <div className="text-slate-500">{block.subtitle}</div>
                                        )}
                                    </td>
                                    ...
                                    <td className="whitespace-nowrap px-6 py-4">
                                        <span
                                            className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${block.isPublished
                                    ...
                                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                        <div className="flex justify-end gap-2">
                                            <Link
                                                href={`/internal/content/${block.id}/edit`}
                                                className="text-blue-600 hover:text-blue-900"
                                            >
                                                Edit
                                            </Link>
                                            <button
                                                onClick={() => togglePublish(block.id, block.isPublished)}
                                                className={`${block.isPublished
